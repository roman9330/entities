...
class ServiceController extends BaseController
{
    ...
    public function updateAction()
    {
        $strErrorDesc = '';
        try {
            $bandModel = new ServiceModel();
            $validator = new requestValidator($this->UPDATE_REQUEST_RULE, $this->params);
            $valid = $validator->validateUpdateRequest();
            if ($valid['success'] === true) {
                $bandModel->updService($this->params);
                $bandModel->updServicei18($this->params);
                $bandModel->updServiceParams($this->params);
            } else {
    ...
    public function getAction()
    {
        ...
        try {
            $bandModel = new ServiceModel();
            $arrBands = $bandModel->getService($this->params);
